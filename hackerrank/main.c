...
struct document get_document(char* text) {
     struct document doc;
     int count=0,p=0,s=0,w=0,c=0;
     while(text[count]!='\0'){
             doc.data=malloc(5*sizeof(struct paragraph));
             doc.paragraph_count=0;
        while(text[count]!='\n'){
            doc.data[p].data=malloc(5*sizeof(struct sentence));
            doc.data[p].sentence_count=0;
            while(text[count]!='.'){
                doc.data[p].data[s].data=malloc(20*sizeof(struct word));
                doc.data[p].data[s].word_count=0;
                while(text[count]!=' '){
                    doc.data[p].data[s].data[w].data=malloc(10*sizeof(char));
                    doc.data[p].data[s].data[w].data[c]=&text[count];
                     count++;
